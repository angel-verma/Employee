...
<form method="POST">
	<table class="table table-bordered table-hover table-condenced">
		...
		<?php
			...
			$query = mysqli_query($conn, "select * from users WHERE status='employee'") or die("Failed to connect");
		...
		<tr>
			...
			<td><img src="<?php echo "uploads/".$data['image']; ?>" alt="" heigth="20" width="20"></td>
			<td><input type="checkbox" name="check[]" value="<?php echo $data['id'];?>"></td>
			<td><a href="update-emp-info.php?uid=<?php echo $data['id']; ?>" class="btn btn-info">Update</a></td>
